...
- `--update-existing deck.pptx` = reconciliation source (the prior generated deck)
- `--template conference-template.pptx` = rendering source (masters, layouts, and `Template[n]`)

`--write-slide-ids` rewrites `deck.md` in place, adding only missing
`<!-- slideId: ... -->` directives and preserving existing ones, so future
updates can match slides by stable identity. During update mode, unmanaged
